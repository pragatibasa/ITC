...
/*	function listadditionalchargemodel($partyid = ''){
	...
	function directbillingbill($billid,$partyid,$pname,$cust_add,$cust_rm,$mat_desc,$thic,$wid,$len,$wei,$inv_no,$rate,$invoicedate,$totalweight_check,$totalrate,$totalamt,$txthandling,$txtadditional_type,$txtamount_mt,$txtoutward_num,$txtscrap,$txtservicetax,$txteductax,$txtsecedutax,$txtgrandtotal,$container,$driverContact){
...
function finalbillgeneratemodel($partyid='',$actualnumberbundle='',$cust_add='',$cust_rm='',$billid='',$gstType, $returnPdf = false) {
	...
	$queryitem = $this->db->query($sqlbundle);
  	...
  	$html .= '
		...
		function recoilpdf($partyid='',$returnPdf = false) {
...
$pdf->Output($pdfname, 'I');
	...
	function billing_direct($billid='',$partyid='',$gstType, $returnPdf = '')
    {
		...
		<td width="30%" align:"center"><h3>Dated: '.$billdate.'</h3></td>
		<td width="30%" align:"left"><h3>EWAYBILL No: '.$inwarddate.'</h3></td>
			</tr>
	</table>';
		...
		<tr>
			<td align="center" width="100%"><hr color=#00CC33 size=5 width=100></td>
					...
					$html .= '<tr>
								...
								<td width="7.96%">'.$totalweight_check.'</td>
								<td width="7.96%">'.$totalweight_check.'</td>
								<td width="7.96%">'.$totalweight_check.'</td>
								<td width="7.96%">'.$totalweight_check.'</td>
								<td width="7.96%">'.$totalweight_check.'</td>
...
if($returnPdf) {
	...
	*/
	...
	$totalrate,$totalamt,$txthandling, $txtadditional_type,$txtamount_mt,$txtoutward_num,$txtscrap,$txtservicetax,$txteductax,$txtsecedutax,$txtgrandtotal ,$container,$txtnsubtotal,$returnPdf=false) {
	$sqlrpt = "select aspen_tblbilldetails.vOutLorryNo as lorryno,
	aspen_tblbilldetails.fTotalWeight as totalweight,
	aspen_tblbilldetails.ntotalpcs as totalpcs,
	aspen_tblbilldetails.ntotalamount as totamount,
